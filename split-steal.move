...
module overmind::split_or_steal {
    ...
    */
    fun init_module(account: &signer) {
        ...
        let (resource) = account::create_resource_account(account, SEED);
        ...
        let state = State {
            next_game_id: 1, // Assuming the first game ID starts from 1
            games: SimpleMap::new(),
            cap: account::get_capability(account),
    ...
    */
    public entry fun create_game(
        ...
        check_if_signer_is_contract_deployer();
        // TODO: Call `check_if_signer_has_enough_apt_coins` function
        check_if_signer_has_enough_apt_coins();
        ...
        let game = Game {
            ...
            player_one: PlayerData {
                ...
                decision_hash: None,
                salt_hash: None,
                decision: DECISION_NOT_MADE, // Assuming DECISION_NOT_MADE is defined somewhere
            },
            player_two: PlayerData {
            ...
            expiration_timestamp_in_seconds: 0, // Assuming the expiration timestamp is set later
        };
        // TODO: Add the game to the State's games SimpleMap instance
        borrow_global_mut<State>(account).games.insert(next_game_id, game);
        // TODO: Transfer `prize_pool_amount` amount of APT from `account` to the resource account
        coin::transfer_from_sender<AptosCoin>(resource_account_address, prize_pool_amount);
        // TODO: Emit `CreateGameEvent` event
        emits(CreateGameEvent(next_game_id));
    }
    ...
    */
    public entry fun submit_decision(
        ...
        check_if_player_participates_in_the_game();
        // TODO: Call `check_if_player_does_not_have_a_decision_submitted` function
        check_if_player_does_not_have_a_decision_submitted();
        // TODO: Set the player's PlayerData decision_hash and salt_hash fields to the values provided in the params
        let mut player_data = findPlayerDataByAddress(player: signer, game_id, player: signer.address());
        player_data.decision_hash.replace(decision_hash);
        player_data.salt_hash.replace(salt_hash);
        // TODO: Emit `SubmitDecisionEvent` event
        emits(SubmitDecisionEvent{
            game_id: game_id,
            player_address: player: signer.address(),
            decision_hash: decision_hash,
            salt_hash: salt_hash,
            event_creation_timestamp_in_seconds: 0 // Assuming the timestamp is set when the event is emitted
        });
    }
    ...
    */
    public entry fun reveal_decision(
        ...
        check_if_both_players_have_a_decision_submitted();
        // TODO: Call `make_decision` function with appropriate PlayerData instance depending on the player's address
        let currentPlayerAddress = player: signer;
        let decision = hash(salt);
        make_decision(currentPlayerAddress, decision, to_bytes(salt));
    
        // TODO: Emit `RevealDecisionEvent` event
        emits(RevealDecisionEvent{
            game_id: game_id,
            player_address: player: signer.address(),
            decision: decision,
            event_creation_timestamp_in_seconds: 0 // Assuming the timestamp is set when the event is emitted
        ...
        let game = borrow_global_mut<Game>(game_id);
        if game.player_one.decision != DECISION_NOT_MADE && game.player_two.decision != DECISION_NOT_MADE {
            // 1) Remove the game from the State's game SimpleMap instance
            borrow_global_mut<State>(player).games.remove(game_id);

            // 2) If both players decided to split, send half of the game's `prize_pool_amount` of APT to both of them
            if game.player_one.decision == DECISION_SPLIT && game.player_two.decision == DECISION_SPLIT {
                let split_amount = game.prize_pool_amount / 2;
                coin::transfer_to_address<AptosCoin>(game.player_one.player_address, split_amount);
                coin::transfer_to_address<AptosCoin>(game.player_two.player_address, split_amount);
            }
            // 3) If one of the players decided to steal and the other one to split, send the game's `prize_pool_amount` of APT to the player that decided to steal
            else if game.player_one.decision == DECISION_STEAL && game.player_two.decision == DECISION_SPLIT {
                coin::transfer_to_address<AptosCoin>(game.player_one.player_address, game.prize_pool_amount);
            }
            else if game.player_one.decision == DECISION_SPLIT && game.player_two.decision == DECISION_STEAL {
                coin::transfer_to_address<AptosCoin>(game.player_two.player_address, game.prize_pool_amount);
            }
            // 4) If both players decided to steal, send the game's `prize_pool_amount` of APT to the deployer of the contract
                ...
                coin::transfer_to_address<AptosCoin>(contract_deployer_address, game.prize_pool_amount);
            }

            emits(ConcludeGameEvent{
                game_id: game_id,
                decision_one: game.player_one.decision,
                decision_two: game.player_two.decision,
                event_creation_timestamp_in_seconds: 0 // Assuming the timestamp is set when the event is emitted
            });
        // Define the function to handle the conclusion of the game when both players have submitted their decisions
        }
    }
    ...
    */
    public entry fun release_funds_after_expiration(_account: &signer, game_id: u128) acquires State {
        ...
        if game.player_one.decision == DECISION_NOT_MADE && game.player_two.decision == DECISION_NOT_MADE {
        // Both players' decisions were not revealed, transfer to the deployer of the contract
        coin::transfer_to_address<AptosCoin>(contract_deployer_address, game.prize_pool_amount);
        } else if game.player_one.decision == DECISION_NOT_MADE {
            // Second player did not reveal their decision, transfer to the first player
            coin::transfer_to_address<AptosCoin>(game.player_one.player_address, game.prize_pool_amount);
        } else if game.player_two.decision == DECISION_NOT_MADE {
            // First player did not reveal their decision, transfer to the second player
            coin::transfer_to_address<AptosCoin>(game.player_two.player_address, game.prize_pool_amount);
        }
        // TODO: Emit `ReleaseFundsAfterExpirationEvent` event
        emits(ReleaseFundsAfterExpirationEvent{
            game_id: game_id,
    ...
    */
    inline fun make_decision(player_data: &mut PlayerData, salt: &String): u64 {
        ...
        check_if_hash_is_correct(player_data.decision_hash, salt);
        ...
        let split_decision = hash::sha3_256(serialize_u64(DECISION_SPLIT) ++ to_bytes(salt));  
        // TODO: Create a SHA3_256 hash of a steal decision from a vector containing serialized DECISION_STEAL const
        //      and bytes of the salt
        let steal_decision = hash::sha3_256(serialize_u64(DECISION_STEAL) ++ to_bytes(salt));
        ...
        //      or DECISION_STEAL depending on the result
         if player_data.decision_hash == Some(split_decision) {
            return DECISION_SPLIT;
        } else if player_data.decision_hash == Some(steal_decision) {
            return DECISION_STEAL;
        } else {
            // If the hash doesn't match either split or steal, it means the decision was not valid.
            // You can handle this case as needed, such as returning a default value or raising an error.
            // For the sake of completion, we return DECISION_NOT_MADE (0) here.
            return DECISION_NOT_MADE;
        }
    }
    ...
    */
    inline fun get_next_game_id(next_game_id: &mut u128): u128 {
        ...
        let current_game_id = *next_game_id;
    ...
    inline fun check_if_signer_is_contract_deployer(signer: &signer) {
        ...
        assert(ResourceExists<State>(contract_deployer_address), 1);
    }
    ...
    inline fun check_if_account_has_enough_apt_coins(account: &signer, amount: u64) {
        ...
        assert(coin::balance<AptosCoin>(account::address()) >= amount, 2);
    }
    ...
    inline fun check_if_game_exists(games: &SimpleMap<u128, Game>, game_id: &u128) {
        ...
        assert(games.contains_key(game_id), 3);
    }
    ...
    inline fun check_if_player_participates_in_the_game(player: &signer, game: &Game) {
        ...
        assert(player::address() == game.player_one.player_address || player::address() == game.player_two.player_address, 4);
    }
    ...
    inline fun check_if_both_players_have_a_decision_submitted(game: &Game) {
        ...
        assert(game.player_one.decision_hash.is_some() && game.player_two.decision_hash.is_some(), 5);
    }
    ...
    inline fun check_if_player_does_not_have_a_decision_submitted(game: &Game, player_address: address) {
        ...
        assert(player_data.decision_hash.is_none(), 6);
    }
    ...
    inline fun check_if_hash_is_correct(hash: vector<u8>, value: vector<u8>) {
        ...
        let computed_hash = hash::sha3_256(value);
        assert(hash == computed_hash, 7);
    }
    ...
    inline fun check_if_game_expired(game: &Game) {
        ...
        let current_timestamp = 0; // Replace this with the actual function to get the current timestamp
        assert(game.expiration_timestamp_in_seconds < current_timestamp, 8);
    }
